...
class ADODB_DataDict {
	...
	function MetaTables()
	{
		...
		$this->connection->raiseErrorFn = false;
		$out = $this->connection->MetaTables();
		$this->connection->raiseErrorFn = 'elxisdbHandler';
		return $out;
	}
	...
	function MetaColumns($tab, $upper=true, $schema=false)
	{
		if (!$this->connection->IsConnected()) { return array(); }
		$this->connection->raiseErrorFn = false;
		$out = $this->connection->MetaColumns($this->TableName($tab), $upper, $schema);
		$this->connection->raiseErrorFn = 'elxisdbHandler';
		return $out;
	}
	...
	function MetaPrimaryKeys($tab,$owner=false,$intkey=false)
	{
		if (!$this->connection->IsConnected()) { return array(); }
		$this->connection->raiseErrorFn = false;
		$out = $this->connection->MetaPrimaryKeys($this->TableName($tab), $owner, $intkey);
		$this->connection->raiseErrorFn = 'elxisdbHandler';
		return $out;
	}
	...
	function MetaIndexes($table, $primary = false, $owner = false)
	{
		if (!$this->connection->IsConnected()) { return array(); }
		$this->connection->raiseErrorFn = false;
		$out = $this->connection->MetaIndexes($this->TableName($table), $primary, $owner);
		$this->connection->raiseErrorFn = 'elxisdbHandler';
		return $out;
	}
	...
	function NameQuote($name = NULL,$allowBrackets=false)
	{
		...
		if (preg_match('/mysql/i', $this->connection->databaseType) && (!preg_match('/^`(.+)`$/', $name))) {
			return $quote.$name.$quote;
		} else {
			return $name;
		}
