...
* @ Modifay for Elxis CMS by Dejan Viduka <dejan@viduka.info>
...
// testing
// ----------------------------------------------
//$params->useLines = "false";
//$params->useIcons = "false";
//$params->openAll = "true";
//
...
$useSelection =  @$params->useSelection ? $params->useSelection : 'true';
$useLines =  @$params->useLines ? $params->useLines : 'true';
$useIcons =  @$params->useIcons ? $params->useIcons : 'true';
$useStatusText =  @$params->useStatusText ? $params->useStatusText : 'false';
$closeSameLevel =  @$params->closeSameLevel ? $params->closeSameLevel : 'false';

// if all folders should be open, we will ignore the closeSameLevel
$openAll =  @$params->openAll ? $params->openAll : 'false';
...
$menus =  @$params->menutype ? explode(",",$params->menutype) : array("mainmenu");
...
if (!@$params->separator && $module->showtitle!=0) {
	$title = $module->title;
	$separator = <<<EOT
...
EOT;
	...
	$separator =  @$params->separator ? $params->separator : "<P>";
...
$database->setQuery( "SELECT m.* FROM #__menu AS m"
	. "\nWHERE menutype='$menu' AND published=1 AND access IN( " . $my->allowed . " )"
...
if ($first_menu_has_been_printed) {
	$content .= $separator;
}
...
$content .= "<link rel=\"styleSheet\" href=\"$mosConfig_live_site/modules/dtree/dtree.css\" type=\"text/css\">\n";
$content .= "<script type=\"text/javascript\" src=\"$mosConfig_live_site/modules/dtree/dtree.js\"></script>\n";
$content .= "<script type=\"text/javascript\">\n";

// create the tree, using the unique name
// pass the live_site parameter on so dTree can find the icons
$content .= "$tree = new dTree('$tree',\"$mosConfig_live_site\");\n";
...
list($baselink,$target) = check_url_target($baselink, $first_row, $lang );
...
$content .= $openAll == "true" ? "$tree.openAll();\n" : "$tree.closeAll();\n";
