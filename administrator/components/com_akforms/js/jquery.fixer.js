...
(function($) {
	...
	// plugin definition
	//
	jQuery.fn.fixer = function(options) {
		var opts = jQuery.extend({}, jQuery.fn.fixer.defaults, options);
		return jQuery(this)
				.each(
						function() {
							// only process tables
							if (jQuery(this).get(0).tagName != "TABLE")
								return;
							// load options
							...
							var sbwidth = jQuery.getScrollbarWidth();
							// get the table into a local variable
							var fixertable = jQuery(this);
							...
							fixershell = jQuery(this.cloneNode(false));
							...
							var rows = jQuery('tr', fixertable);
							var cols = jQuery('th,td', rows[0]);
							...
							rows.each(function() {
										if (jQuery(this).is(":hidden"))
											jQuery(this).evenIfHidden(
															function(element) {
																heights.push(element.height());
																outerheights.push(element.outerHeight());
															});
										else {
											outerheights.push(jQuery(this).outerHeight());
							...
							if (fixcols) {
								if (fixrows) {
									...
									for (i = 0; i < fixedrows; i++) {
										row = jQuery(rows[i].cloneNode(false));
										row.append(jQuery('td,th', rows[i]).slice(0,
												fixedcols));
							...
							if (fixrows) {
									...
									for (i=fixedcols;i<colcount;i++)
										colgroup.append(jQuery("<col></col>").css("width",widths[i]));
							...
							if (fixrows) {
								...
								wrapper.append(jQuery("<div></div>").css("clear","both").css('margin', '0').css('padding', '0'));
							}
							// wrap the bottom left table if it exists
							if (fixcols) {
								var bottomleft = jQuery("<div></div>");
							...
							var bottomright = jQuery("<div></div>");
							...
							if (tableheight>height) {
								...
								if (!setwidth && fixrows) {
									...
									if (fixedheight< (2*sbwidth)) {
										topright.prepend(jQuery("<div></div>").css("height",2*sbwidth-fixedheight));
										if (fixcols)
											topleft.prepend(jQuery("<div></div>").css("height",2*sbwidth-fixedheight));
									}
								}
							}
							if (jQuery.browser.msie)
							...
							wrapper.append(jQuery("<div></div>").css("clear","both"));
  ...
  return this.each( function() {
    ...
    hiddenElements.each( function() {
	  ...
	  if (tag=='TABLE' || tag=='TBODY')
		  ...
		  display = 'table-cell';
      jQuery(this).attr( 'style', style + ' display: '+display+' !important;' );
